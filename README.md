...
- 会員種別: 新規登録では一般会員（年会費無料）とプレミアム会員（年会費2,000円/PayPay決済案内）を選択できます。既存番号OKP-0001〜0215はプレミアム会員として扱います。
...
現在のGoogleフォーム会員データは215名分（`OKP-0001` から `OKP-0215`）です。Googleフォームでは同じメールアドレスを複数会員が使っている場合があるため、`legacy_members` は `member_id` を主キーにし、メールアドレスの重複を許可します。
