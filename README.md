...
- Googleフォーム既存会員ID引き継ぎ: `OKP-0001`形式の既存番号をSupabase登録時に照合して引き継ぎます。
...
既にGoogleフォームで登録済みの方は、フォームで発行された `OKP-0001` 形式の番号をアプリ登録時に引き継げます。
...
5. 既存会員は、会員登録画面でGoogleフォーム登録時のメールアドレスと会員IDを入力します。
