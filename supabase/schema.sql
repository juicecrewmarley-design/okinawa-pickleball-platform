...
begin
  ...
  if requested_legacy_member_id is not null and legacy_match.member_id is null then
  ...
  final_full_name := coalesce(legacy_match.full_name, nullif(new.raw_user_meta_data ->> 'full_name', ''), split_part(new.email, '@', 1));
  final_furigana := coalesce(legacy_match.furigana, nullif(new.raw_user_meta_data ->> 'furigana', ''), '');
  final_gender := coalesce(legacy_match.gender, coalesce(nullif(new.raw_user_meta_data ->> 'gender', ''), 'no_answer')::public.gender_type);
  final_birth_date := coalesce(legacy_match.birth_date, nullif(new.raw_user_meta_data ->> 'birth_date', '')::date);
  final_phone := coalesce(legacy_match.phone, nullif(new.raw_user_meta_data ->> 'phone', ''));
  final_area := coalesce(legacy_match.area, coalesce(nullif(new.raw_user_meta_data ->> 'area', ''), 'other')::public.member_area);
  final_residence_scope := coalesce(legacy_match.residence_scope, coalesce(nullif(new.raw_user_meta_data ->> 'residence_scope', ''), 'okinawa')::public.residence_scope);
  final_municipality := coalesce(legacy_match.municipality, nullif(new.raw_user_meta_data ->> 'municipality', ''));
