...
既にGoogleフォームで会員登録している方は、フォームで発行済みの `OKP-0001` 形式の会員IDをそのまま引き継げます。
...
2. 既存会員がアプリの会員登録画面で、Googleフォーム登録時のメールアドレスと会員IDを入力します。
3. Supabase Authでユーザーが作成される時、DBトリガーが `legacy_members` を照合します。
4. メールアドレスと会員IDが一致した場合、`profiles.member_id` に既存の `OKP-0001` 形式の番号を引き継ぎます。
...
これで既存Googleフォーム会員は、同じメールアドレスと会員IDでアプリ登録した時に、既存番号を引き継げます。
